...
import { FaCircleNotch, FaChevronDown } from "react-icons/fa";
...
export const ResultsGrid: React.FC<ResultsGridProps> = ({
  ...
  // Track expanded state per scene
    ...
    <div className="flex flex-col gap-8 pb-32">
      {scenes.map((scene) => {
        ...
        const videosToShow = isExpanded
          ? scene.options
          : scene.options.slice(0, INITIAL_VISIBLE_VIDEOS);
        const hasMoreToExpand = scene.options.length > INITIAL_VISIBLE_VIDEOS;
                ...
                <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-vf-lime scrollbar-track-neutral-900">
                  {videosToShow.length > 0 ? (
                    <>
                      ...
                      {/* Expand / Collapse Local */}
                      {hasMoreToExpand && !isExpanded && (
                        <div className="shrink-0 w-[100px] flex items-center justify-center">
                          <button
                            onClick={() => toggleExpand(scene.id)}
                            className="text-xs font-mono text-neutral-500 hover:text-white border border-dashed border-neutral-700 hover:border-white rounded-lg w-full h-[158px] flex flex-col items-center justify-center gap-2 transition-all"
                          >
                            <FaChevronDown className="text-lg" />
                            SHOW {scene.options.length -
